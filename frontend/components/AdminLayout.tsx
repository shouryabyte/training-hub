...
export const AdminLayout: React.FC = () => {
  ...
  if (!token) return <Navigate to={`/auth?returnTo=${encodeURIComponent('/admin')}`} replace />;
  if (role !== 'ADMIN') return <Navigate to={role === 'TEACHER' ? '/teacher' : '/dashboard'} replace />;
    ...
    <div className="min-h-screen selection:bg-indigo-500 selection:text-white bg-[#020617]">
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          >
            ...
            <div className="leading-tight text-left">
              ...
              <div className="mono text-[10px] font-black text-indigo-300 uppercase tracking-[0.35em]">Admin Console</div>
            </div>
          </button>

          <div className="flex items-center gap-3">
            <div className="hidden sm:block text-right">
              <div className="text-sm font-black text-white leading-tight">{user?.name || 'Admin'}</div>
      ...
      <footer className="border-t border-white/5 bg-slate-950/30">
        <div className="max-w-7xl mx-auto px-6 py-6 text-xs text-slate-500 flex items-center justify-between">
          <span>© {new Date().getFullYear()} Nexchakra</span>
