...
export const AlphaPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const divisions = (catalog.divisions || []).filter((d: any) => String(d?.batch?.name) === 'ALPHA');
        if (mounted) {
  ...
  return (
    <div className="px-6 pb-20">
      <div className="max-w-7xl mx-auto">
        <div className="mb-12">
          <p className="mono text-[10px] font-black text-indigo-400 uppercase tracking-[0.4em] mb-4">Division 01 • Alpha</p>
          <h1 className="text-4xl md:text-6xl font-black text-white tracking-tighter">
            Class 11 & 12 <span className="text-slate-400">Foundation</span>
          </h1>
          <p className="text-slate-400 mt-4 font-medium max-w-3xl">
            Preparing the global elite for top-tier academic entries — with clarity, mentorship cadence, and measurable progress.
          </p>
        </div>

        <div className="grid lg:grid-cols-2 gap-6">
          <div className="glass-card rounded-[2.5rem] border-white/10 p-8">
            <h2 className="text-2xl font-black text-white tracking-tight mb-4">Vision & Outcomes</h2>
            <ul className="space-y-3 text-slate-400 font-medium text-sm">
              {[
                'Find your direction early: interests → role map → weekly milestones.',
                'Build a credible profile: writing, projects, and mentor reviews.',
                'Execute with cadence: accountability loops and feedback cycles.',
              ].map((t) => (
          ...
          <div className="glass-card rounded-[2.5rem] border-white/10 p-8">
            <h2 className="text-2xl font-black text-white tracking-tight mb-4">Target Audience</h2>
            <p className="text-slate-400 font-medium">
              Class 11, Class 12, and foundation students who want a structured, mentor-led system instead of guesswork.
            </p>
          </div>
        </div>
        ...
        <div className="mt-12" id="divisions">
          <div className="flex items-end justify-between gap-6 mb-6">
            <div>
              <h2 className="text-2xl font-black text-white tracking-tight">Alpha Divisions</h2>
              <p className="text-slate-400 font-medium mt-2">Premium tracks with brief, high-signal descriptions.</p>
            </div>
            <a
              href="#pricing"
          ...
          {loading ? (
            ...
            <div className="text-slate-400 font-medium">
              No divisions found. Ask a teacher to create Alpha courses, or check your backend seed.
            </div>
          )}
        </div>

        <div className="mt-14" id="pricing">
          <h2 className="text-2xl font-black text-white tracking-tight mb-6">Alpha Plans</h2>
          {loading ? (
            ...
            <div className="text-slate-400 font-medium">No active Alpha plans yet.</div>
          )}
